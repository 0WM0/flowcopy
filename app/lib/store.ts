...
const sanitizeTermRegistry = (value: unknown): TermRegistryEntry[] => {
  if (!Array.isArray(value)) {
    return [];
  }

  const entries: TermRegistryEntry[] = [];
  ...
  value.forEach((item) => {
    ...
    const canvasPosition =
      ...
      typeof source.canvasPosition === "object" &&
      typeof (source.canvasPosition as { x?: unknown }).x === "number" &&
      typeof (source.canvasPosition as { y?: unknown }).y === "number"
        ? {
            x: (source.canvasPosition as { x: number }).x,
            y: (source.canvasPosition as { y: number }).y,
          }
        : null;
    ...
    entries.push({
      ...
      assignedNodeId:
        typeof source.assignedNodeId === "string"
          ? source.assignedNodeId
          : null,
      assignedSlotId:
        typeof source.assignedSlotId === "string"
          ? source.assignedSlotId
          : null,
      canvasPosition,
      isAutoGenerated: source.isAutoGenerated === true,
      deduplicationSuffix:
        typeof source.deduplicationSuffix === "string"
          ? source.deduplicationSuffix
          : null,
      createdAt:
