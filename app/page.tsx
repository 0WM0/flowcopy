...
export default function Page() {
  ...
  const handleFloatingTermCancel = useCallback((entryId: string) => {
    setTermRegistry((prev) => prev.filter((entry) => entry.id !== entryId));
    setEditingFloatingTermId(null);
  }, []);
    ...
    () =>
      termRegistry
        ...
        .map((entry) => ({
          ...
          data: {
            value: entry.value,
            editing: editingFloatingTermId === entry.id,
            onCommit: (value: string) => handleFloatingTermCommit(entry.id, value),
            onCancel: () => handleFloatingTermCancel(entry.id),
          },
          draggable: false,
          selectable: false,
        })),
    [
      editingFloatingTermId,
      handleFloatingTermCancel,
      handleFloatingTermCommit,
      termRegistry,
    ]
  );
