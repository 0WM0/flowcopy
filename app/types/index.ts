...
export type TermRegistryEntry = {
  ...
  /**
   * Canvas position when rendered as a pill (Express Mode).
   * Non-null when the entry is unassigned (assignedNodeId and assignedSlotId both null)
   * and has been placed directly on the canvas. Null when the entry has no canvas presence
   * or when it has been assigned to a card slot (in which case it renders inside its card).
   */
  canvasPosition: { x: number; y: number } | null;

  /**
   * Flag set when the entry was created as a blank placeholder pill via rapid Tab entry
   * (value auto-generated as "Term 1", "Term 2", etc.). Used for audit filtering and
   * optional auto-cleanup on exit. False for entries created with user-entered content.
   */
